...
const DeleteTopicDialog = ({
  ...
  const [openConfirmDeleteTopicDialog, setOpenConfirmDeleteTopicDialog] = useState(false)
  ...
  const handleChangeSubtopic = (e) => {
    ...
    if (e.target.checked) {
      ...
      if (checkedSubtopic.some(item => item.topic_id === allTopics[id].topic_id)) {
        setCheckedSubtopic((prev) => prev.filter((s) => s.topic_id !== allTopics[id].topic_id))
      }
    }
  };
