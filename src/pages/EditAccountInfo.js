...
const EditAccountInfo = () => {
  ...
  const [showPrompt, confirmNav, cancelNav] =
    useLeavingDialogPrompt(canShowPrompt);
  ...
  const [emailConfirmDialogState, setEmailConfirmDialogState] =
    useState("verifyEmail");
  ...
  function convertAcronymToProvince(acronym) {
    let province;
    if (acronym === "AB") {
      province = I18n.get("ab");
    } else if (acronym === "BC") {
      province = I18n.get("bc");
    } else if (acronym === "MB") {
      province = I18n.get("mb");
    } else if (acronym === "NB") {
      province = I18n.get("nb");
    } else if (acronym === "NL") {
      province = I18n.get("nl");
    } else if (acronym === "NT") {
      province = I18n.get("nt");
    } else if (acronym === "NS") {
      province = I18n.get("ns");
    } else if (acronym === "NU") {
      province = I18n.get("nu");
    } else if (acronym === "ON") {
      province = I18n.get("on");
    } else if (acronym === "PE") {
      province = I18n.get("pe");
    } else if (acronym === "QC") {
      province = I18n.get("qc");
    } else if (acronym === "SK") {
      province = I18n.get("sk");
    } else {
      province = I18n.get("yt");
    }
    return province;
  }
  ...
  async function getUserData() {
    try {
      ...
      if (
        user.data.getUserByEmail.email_notice &&
        user.data.getUserByEmail.sms_notice
      ) {
  ...
  async function updateDatabase() {
    ...
    if (
      ...
      let getUserTopics = await API.graphql(
      ...
      if (userTopics.length !== 0) {
        for (let i = 0; i < userTopics.length; i++) {
          await API.graphql(
            graphqlOperation(userUpdateChannelPrefrence, {
              user_id: userData.user_id,
  ...
  return (
    <>
      {defaultNotificationError ? (
        <Collapse in={defaultNotificationError}>
          <Alert
            severity={"error"}
            onClose={() => setDefaultNotificationError(false)}
          >
            {alertContent}
          </Alert>
        </Collapse>
