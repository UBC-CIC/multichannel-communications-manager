...
```bash
aws cloudformation deploy --template-file cfn-amplifyRole.yaml --stack-name amplifyconsole-mcm-backend-role --capabilities CAPABILITY_NAMED_IAM
```
...
This step creates the IAM role called **amplifyconsole-mcm-backend-role** that will be used on the next step.
...
2. Select `Manage User Pools`, then select the user pool that begins with mcmf0082716
...
2. Search for `CreateAuthChallenge` and select the one that was created for the newly deployed project (e.g. mcmf0082716f0082716CreateAuthChallenge-devp)![alt text](images/deployment/lambda-02.PNG)
...
5. Select `Manage User Pools` and select the user pool that begins with mcmf0082716.
