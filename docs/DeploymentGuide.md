...
```bash
aws cloudformation deploy --template-file cfn-amplifyRole.yaml --stack-name amplifyconsole-communicationsmanager-backend-role --capabilities CAPABILITY_NAMED_IAM
```
...
This step creates the IAM role called **amplifyconsole-communicationsmanager-backend-role** that will be used on the next step.
...
2. Select the **amplifyconsole-communicationsmanager-backend-role** we made previously for the deployment role. Expand the "Environment variables" section and copy paste the following variable value pairs: `_LIVE_UPDATES`:`[{"name":"Amplify CLI","pkg":"@aws-amplify/cli","type":"npm","version":"10.0.0"}]`. Then press the orange button at the bottom of the page, and then press `Continue` when that pops up![alt text](images/deployment/deployment-02.PNG)
...
2. Select `Manage User Pools`, then select the user pool that begins with communicationsmanagerf0082716
...
2. Search for `CreateAuthChallenge` and select the one that was created for the newly deployed project (e.g. communicationsmanagerf0082716f0082716CreateAuthChallenge-devp)![alt text](images/deployment/lambda-02.PNG)
...
5. Select `Manage User Pools` and select the user pool that begins with communicationsmanagerf0082716.
